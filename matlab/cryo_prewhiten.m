...
for idx=1:n
    if mod(L,2)==1 % Odd-sized image
        pp(k(1)-l(1):k(1)+l(1), k(2)-l(2):k(2)+l(2))=proj(:,:,idx); % Zero pad the image to twice the size.
    else
        pp(k(1)-l(1):k(1)+l(1)-1, k(2)-l(2):k(2)+l(2)-1)=proj(:,:,idx); % Zero pad the image to twice the size.
    end
    ...
    if mod(L,2)==1
        p2(:,:,idx) = pp2(k(1)-l(1):k(1)+l(1), k(2)-l(2):k(2)+l(2));
    else
        p2(:,:,idx) = pp2(k(1)-l(1):k(1)+l(1)-1, k(2)-l(2):k(2)+l(2)-1);
    end
